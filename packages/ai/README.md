...
```bash
npm install @dg-claw/ai
```

TypeBox exports are re-exported from `@dg-claw/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@dg-claw/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@dg-claw/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@dg-claw/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@dg-claw/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@dg-claw/ai';
...
```typescript
import { getModel, stream } from '@dg-claw/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@dg-claw/ai';
...
```typescript
import { Model, stream } from '@dg-claw/ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@dg-claw/ai';
...
```typescript
import { getModel, complete, Context } from '@dg-claw/ai';
...
```typescript
import { Context, getModel, complete } from '@dg-claw/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@dg-claw/ai/oauth` entry point in Node.js.
...
Set `DG_CLAW_AI_ANTIGRAVITY_VERSION` to override the Antigravity User-Agent version when Google updates their requirements:

```bash
export DG_CLAW_AI_ANTIGRAVITY_VERSION="1.23.0"
```
...
```typescript
import { getEnvApiKey } from '@dg-claw/ai';
...
```bash
npx @dg-claw/ai login              # interactive provider selection
npx @dg-claw/ai login anthropic    # login to specific provider
npx @dg-claw/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@dg-claw/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@dg-claw/ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@dg-claw/ai/oauth';
...
```typescript
import { getModel, complete } from '@dg-claw/ai';
import { getOAuthApiKey } from '@dg-claw/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@dg-claw/ai`
