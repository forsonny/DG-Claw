...
Stateful agent with tool execution and event streaming. Built on `@dg-claw/ai`.

## Installation

```bash
npm install @dg-claw/agent
```
...
```typescript
import { Agent, streamProxy } from "@dg-claw/agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@dg-claw/agent";
